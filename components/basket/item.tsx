...
const Item: FunctionComponent<IProps> = ({ id, name, count, update }) => {
  return <ListItem button>
    ...
    <ListItemSecondaryAction>
      <IconButton onClick={() => { update(id, -1); }} edge="end" aria-label="remove">
        <Remove />
      </IconButton>
      <IconButton edge="end" aria-label="add">
        {count}
      </IconButton>
      <IconButton onClick={() => { update(id, 1); }} edge="end" aria-label="add">
        <Add />
      </IconButton>
    </ListItemSecondaryAction>
  </ListItem>;
